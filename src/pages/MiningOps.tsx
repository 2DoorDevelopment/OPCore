...
import { Pickaxe, Plus, Hash } from 'lucide-react'
...
import { createMiningSession, getSessionCodes, getSession } from '../lib/firestore'
...
export function MiningOps() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!cancelled) {
        setSessions(docs.filter((s): s is Session => s !== null))
        setLoading(false)
      }
    }
    load()
  ...
  async function handleCreate() {
    ...
    setCreateError(null)
    try {
      const code = await createMiningSession(
        user.uid,
            ...
            <Card>
              <CardContent>
                <p className="text-text-muted text-sm text-center py-8">
                  No sessions yet. Create one or join a crew with their code.
                </p>
              </CardContent>
            </Card>
          )}
